...
<xsl:template match="/">
 ...
 <xsl:if test="ckbk:substring-before-last2('abc.def.gh','.') ne 'abc.def' ">
    <xsl:message terminate="yes">
...
<xsl:function name="ckbk:substring-before-last2">
	<xsl:param name="input"/>
	<xsl:param name="substr"/>
	<xsl:param name="mask-regex" select="true()"/>
	...
	<xsl:sequence 
       select="if ($substr) 
               then 
                  if (contains($input, $ matchstr)) then 
                  string-join(tokenize($input, $matchstr)
                    [position() ne last()],$matchstr) 
                  else ''
               else $input"/>
</xsl:function>


<xsl:function name="ckbk:substring-after-last2">
	<xsl:param name="input"/>
	<xsl:param name="substr"/>
	<xsl:param name="mask-regex" select="true()"/>
	<xsl:variable name="matchstr" 
    ...
    select="if ($substr) 
            then
               if (contains($input, $matchstr))
               then tokenize($input, $matchstr)[last()] 
               else '' 
            else $input"/>
</xsl:function>
